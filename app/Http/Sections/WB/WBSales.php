 ...
 * Class WBSales
 *
 * @property \App\Models\WBParser\WBSale $model
 *
 * @see http://sleepingowladmin.ru/docs/model_configuration_section
 */
class WBSales extends Section implements Initializable
{
    ...
    /**
     * @var \App\Models\WBParser\WBSale
     */
    protected $model = WBSale::class;
        ...
        app()->booted(
            function () {
                \AdminNavigation::getPages()->findById('site_wb')
                    ->addPage($this->makePage(4)->setIcon('fa fa-newspaper-o'));

            }
        );

        $this->updating(function ($config, \App\Models\WBParser\WBSale $model) {
        });

        $this->creating(function ($config, \App\Models\WBParser\WBSale $model) {
        });

        $this->created(function ($config, \App\Models\WBParser\WBSale $model) {
        });
    }
    ...
    public function onDisplay()
    {
        ...
        $active = AdminDisplay::datatablesAsync()
            ...
            ->setColumns(
                ...
                AdminColumn::text('sale_at', 'Дата продажи')->setWidth('150px')
                    ->setSearchCallback(function ($column, $query, $search) {
                        // Из-за ошибки поиска кирилицы в данных полях(int, date)
    ...
    public function onEdit($id)
    {
        ...
        $main->setItems(
            AdminFormElement::columns()
                ...
                ->addColumn([
                    AdminFormElement::text('regionName', 'Регион')
                        ->setReadonly(1),
                ], 4)
                ->addColumn([
                ...
                ->addColumn([
                    AdminFormElement::text('totalPrice', 'Начальная розничная цена товара')
                        ->setReadonly(1),
                ], 4)
                ->addColumn([
                    AdminFormElement::text('discountPercent', 'Согласованная скидка на товар')
                        ->setReadonly(1),
                ], 4)
                ...
                ->addColumn([
                    AdminFormElement::text('forPay', 'К перечислению поставщику')
                        ->setReadonly(1),
                ], 4)
                ->addColumn([
                    AdminFormElement::text('finishedPrice', 'Фактическая цена из заказа(с учетом всех скидок, включая и от WB)')
                        ->setReadonly(1),
                ], 4)
                ->addColumn([
                    AdminFormElement::text('priceWithDisc', 'Цена от которой считается вознаграждение поставщика forpay(с учетом всех согласованных скидок)')
                        ->setReadonly(1),
                ], 4)
                ...
                ->addColumn([
                    AdminFormElement::text('saleID', 'Уникальный идентификатор продажи\ возврата(SXXXX-продажа,RXXXX- возврат,DXXXX-доплата)')
                        ->setReadonly(1),
                ], 4)
                ->addColumn([
                    AdminFormElement::text('sale_at', 'Дата продажи')
                        ->setReadonly(1),
                ], 4)
                ->addColumn([], 12)
